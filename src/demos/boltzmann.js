import { createCanvas, addHint, addControls, rand, randInt, TAU, dist } from '/src/lib/shared.js'
...
export function mountBoltzmann(containerId = 'demo-boltz') {
  ...
  const N = 8
  let states2 = Array(N).fill(0).map(() => Math.random() > .5 ? 1 : 0)
  ...
  function draw() {
    ctx.clearRect(0, 0, 750, 320); let E = energy()
    let cx2 = 180, cy2 = 148, rad = 100
    for (let i = 0; i < N; i++) for (let j = i + 1; j < N; j++) {
      let ai = i / N * TAU - Math.PI / 2, aj = j / N * TAU - Math.PI / 2
    ...
    for (let i = 0; i < N; i++) {
      let a = i / N * TAU - Math.PI / 2, x = cx2 + Math.cos(a) * rad, y = cy2 + Math.sin(a) * rad
    ...
    if (energyHist.length > 1) {
      let mx2 = Math.max(...energyHist), mn = Math.min(...energyHist); if (mx2 === mn) mx2 = mn + 1
  ...
  c.onclick = e => {
    let r2 = c.getBoundingClientRect(), mx2 = (e.clientX - r2.left) * 750 / r2.width, my = (e.clientY - r2.top) * 320 / r2.height
    for (let i = 0; i < N; i++) {
      let a = i / N * TAU - Math.PI / 2, x = 180 + Math.cos(a) * 100, y = 148 + Math.sin(a) * 100
  ...
  document.getElementById('bt').oninput = e => { temp = e.target.value / 10; document.getElementById('btv').textContent = temp.toFixed(1); draw() }
  ...
  rst.onclick = () => { states2 = Array(N).fill(0).map(() => Math.random() > .5 ? 1 : 0); energyHist = []; temp = 2; document.getElementById('bt').value = 20; document.getElementById('btv').textContent = '2.0'; if (tmr) { clearInterval(tmr); tmr = null; ab.classList.remove('active') } draw() }
  ctrl.appendChild(sb); ctrl.appendChild(ab); ctrl.appendChild(rst)
  draw()
...
export function unmountBoltzmann() { if (cleanup) cleanup() }

export default { mountBoltzmann, unmountBoltzmann }
