...
export function mountNaiveBayes(containerId = 'demo-nb') {
  ...
  const words = [
    { w: 'free', spam: .85, ham: .05 },
    { w: 'money', spam: .7, ham: .08 },
    { w: 'hello', spam: .15, ham: .6 },
    { w: 'meeting', spam: .05, ham: .55 },
    { w: 'winner', spam: .8, ham: .02 },
    { w: 'project', spam: .08, ham: .5 },
    { w: 'click', spam: .75, ham: .06 },
    { w: 'dear', spam: .2, ham: .4 }
  ]
  let selected = new Set([0, 4, 6])
  function calc() {
    let lS = Math.log(.3), lH = Math.log(.7)
    words.forEach((w, i) => {
      if (selected.has(i)) {
        lS += Math.log(w.spam); lH += Math.log(w.ham)
      } else {
        lS += Math.log(1 - w.spam); lH += Math.log(1 - w.ham)
      }
    })
    let mx2 = Math.max(lS, lH), eS = Math.exp(lS - mx2), eH = Math.exp(lH - mx2)
    return eS / (eS + eH)
  }
  function draw() {
    ...
    words.forEach((w, i) => {
      let col = i % 4, row = Math.floor(i / 4), x = 15 + col * 183, y = 28 + row * 52
      ...
      ctx.fillText('P(w|spam)=' + (w.spam * 100 | 0) + '%', x + 8, y + 36)
      ctx.fillText('P(w|ham)=' + (w.ham * 100 | 0) + '%', x + 98, y + 36)
      if (on) {
        ctx.fillStyle = w.spam > w.ham ? '#ff6b6b' : '#4ecdc4'; ctx.font = '600 9px Fira Code'; ctx.fillText(w.spam > w.ham ? '↑SPAM' : '↑HAM', x + 140, y + 20)
      }
    })
  ...
  c.onclick = e => {
    let r = c.getBoundingClientRect(), mx2 = (e.clientX - r.left) * 750 / r.width, my = (e.clientY - r.top) * 320 / r.height
    words.forEach((w, i) => {
      let col = i % 4, row = Math.floor(i / 4), x = 15 + col * 183, y = 28 + row * 52
      if (mx2 >= x && mx2 <= x + 176 && my >= y && my <= y + 44) {
        if (selected.has(i)) selected.delete(i); else selected.add(i); draw()
      }
    })
  }
  const ctrl = addControls(el)
  const rst = document.createElement('button'); rst.className = 'btn'; rst.textContent = '↻ Reset'
  rst.onclick = () => { selected = new Set([0, 4, 6]); draw() }
...
export function unmountNaiveBayes() { if (cleanup) cleanup() }

export default { mountNaiveBayes, unmountNaiveBayes }
