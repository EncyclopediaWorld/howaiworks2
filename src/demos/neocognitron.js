...
export function mountNeocognitron(containerId = 'demo-neo') {
  const __id = containerId || 'demo-neo';
const el = $(__id);
if (!el) return () => {};
el.innerHTML = '';
...
ctx.fillText('Σ='+total.toFixed(0),sx+60+fm[0].length*fcs+4,fy+10)});
// C-cells: pooled
let cx2=540;
...
ctx.fillText('→',ox+G*cs+8,oy+G*cs/2);ctx.fillText('→',sx+215,oy+G*cs/2);
// Architecture label
