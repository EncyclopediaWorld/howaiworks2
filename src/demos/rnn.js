...
export function mountRnn(containerId = 'demo-rnn') {
  ...
  function draw() {
    ...
    sent.words.forEach((w, i) => {
      let x = startX + i * cellW + i * 8, fed = i <= pos, isCur = i === pos
      ...
      if (fed) {
        ctx.strokeStyle = isCur ? '#38bdf8' : '#38bdf844'; ctx.lineWidth = 1.5
    ...
    if (history.length > 0) {
      let barW = Math.min(85, 660 / history.length)
      history.forEach((h, i) => {
        let x = 20 + i * (barW + 6), y = hy + 8
    ...
    ctx.fillStyle = '#7d7a8c'; ctx.font = '400 9px Fira Code'; ctx.fillText('Each step blends the current input with memory of ALL previous inputs', dx + 240, dcy + 14)
  ...
  ctrl.appendChild(fb); ctrl.appendChild(ab); ctrl.appendChild(nb); ctrl.appendChild(rst)
  draw()
  cleanup = () => { try { el.innerHTML = '' } catch (e) { } }
  return cleanup
}

export function unmountRnn() { if (cleanup) cleanup() }

export default { mountRnn, unmountRnn }
