...
export function mountBayes(containerId = 'demo-bayes') {
  const __id = containerId || 'demo-bayes';
const el = $(__id);
if (!el) return () => {};
el.innerHTML = '';
...
ctx.fillText(post<15?'😌 Relax! You\'re almost certainly fine.':
post<35?'🤔 Unlikely sick, but maybe retest.':
post<65?'⚠️ Coin-flip. Further testing needed.':
post<85?'😟 Likely sick. See a doctor.':
'🚨 Very likely sick. Seek treatment.',400,188);
// Formula
...
$('bp').oninput=e=>{prev=+e.target.value/100;$('bpv').textContent=(prev*100|0)+'%';draw()};
