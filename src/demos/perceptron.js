...
export function mountPerceptron(containerId = 'demo-perceptron') {
  ...
  function ny(py) { return py / 340 }
  function seed() {
    pts = []
    for (let i = 0; i < 8; i++) { pts.push({ x: rand(60, 320), y: rand(40, 300), c: 0 }); pts.push({ x: rand(430, 700), y: rand(40, 300), c: 1 }) }
  ...
  function trainEpoch() {
    errs = 0; epoch++;
    let sh = [...pts].sort(() => Math.random() - .5)
    sh.forEach(p => {
      let pred = predict(p.x, p.y), err = p.c - pred
  ...
  function draw() {
    ...
    if (epoch > 0 && (Math.abs(w1) > 1e-8 || Math.abs(w2) > 1e-8)) {
      ...
      } else { let xv = -b / w1 * 750; ctx.moveTo(xv, 0); ctx.lineTo(xv, 340) }
      ctx.stroke(); ctx.restore()
    }
    pts.forEach(p => {
      let pred = predict(p.x, p.y), wrong = pred !== p.c
      ctx.save(); if (!wrong) { ctx.shadowColor = p.c ? '#38bdf8' : '#ff6b6b'; ctx.shadowBlur = 8 }
      ctx.beginPath(); ctx.arc(p.x, p.y, wrong ? 9 : 7, 0, Math.PI * 2); ctx.fillStyle = p.c ? '#38bdf8' : '#ff6b6b'; ctx.fill()
  ...
  let tmr = null; const ab = document.createElement('button'); ab.className = 'btn'; ab.textContent = '⏩ Auto Train'; ab.onclick = () => { if (tmr) { clearInterval(tmr); tmr = null; ab.classList.remove('active') } else { tmr = setInterval(() => { trainEpoch(); draw(); if (errs === 0) { clearInterval(tmr); tmr = null; ab.classList.remove('active') } }, 60); ab.classList.add('active') } }
...
export function unmountPerceptron() { if (cleanup) cleanup() }

export default { mountPerceptron, unmountPerceptron }
