...
export function mountDecisionTree(containerId = 'demo-tree') {
  const __id = containerId || 'demo-tree';
const el = $(__id);
if (!el) return () => {};
el.innerHTML = '';
...
ctx.fillText(node.axis+'≤'+node.value.toFixed(0),lx+3,ly);
...
ctx.fillText(n.axis+'≤'+n.value.toFixed(0),tn.x,tn.y+3);ctx.textAlign='left'}});
// Stats
...
ctx.fillText('Split criterion: Gini impurity = 1 − Σpᵢ²',400,py+88)}
