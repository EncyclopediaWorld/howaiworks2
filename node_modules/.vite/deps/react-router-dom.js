...
function createRouter(init) {
	...
	async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, pendingActionData, pendingError) {
		...
		if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
			...
			}, pendingActionData ? { actionData: pendingActionData } : {}, updatedFetchers ? { fetchers: new Map(state.fetchers) } : {}));
			return { shortCircuited: true };
		}
		if (!isUninterruptedRevalidation) {
	...
	*/
	async function startRedirectNavigation(state, redirect, _temp) {
		...
		let redirectLocation = createLocation(state.location, redirect.location, _extends$2({ _isRedirect: true }, isFetchActionRedirect ? { _isFetchActionRedirect: true } : {}));
...
async function callLoaderOrAction(type, request, match, matches, manifest, mapRouteProperties, basename, opts) {
	...
	try {
		...
		if (match.route.lazy) if (handler) result = (await Promise.all([runHandler(handler), loadLazyRouteModule(match.route, mapRouteProperties, manifest)]))[0];
		else {
...
function BrowserRouter(_ref) {
	let { basename, children, future, window } = _ref;
	let historyRef = import_react.useRef();
...
function HashRouter(_ref2) {
	let { basename, children, future, window } = _ref2;
	let historyRef = import_react.useRef();
